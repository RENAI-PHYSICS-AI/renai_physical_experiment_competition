...
截至 2026 年 8 月 21 日，两套答辩材料均已按最终单文件版界面更新。普通答辩 PPT 适合现场人工讲解；自动配音版内嵌逐页音频和换页计时，声速采用女声，李萨如采用男声。两段成品视频均带硬字幕，时长约 9 分 49 秒，严格控制在 10 分钟以内。

| 项目 | 幻灯片 | 配音 | 成品视频 |
| --- | ---: | --- | ---: |
| 声速测量 | 12 页 | 女声，12 页均内嵌音频与自动换页计时 | 约 9 分 49 秒 |
| 李萨如图形 | 12 页 | 男声，12 页均内嵌音频与自动换页计时 | 约 9 分 49 秒 |

### 自动播放说明

自动播放功能位于文件名含“自动配音版”的 PPTX 中，不在普通“答辩PPT”文件中。请在 Microsoft PowerPoint 中选中第 1 页后按 `F5`，进入幻灯片放映模式；配音会随第 1 页开始播放，结束后按预设时间自动翻页。仅在编辑界面打开文件不会自动开始，放映期间点击、切换窗口或退出放映也可能中断音频。若现场只需稳定播放，建议直接使用已生成的 MP4。
...
- [自动配音版 PPT（女声、自动翻页）](声速/设计报告/声速测量实验智能助教_自动配音版.pptx)
...
- [自动配音版 PPT（男声、自动翻页）](李萨如/设计报告/李萨如图形实验智能助教_自动配音版.pptx)
