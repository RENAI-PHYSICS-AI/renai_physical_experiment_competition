...
function defaultRuntimeNodeModules() {
  if (process.env.CODEX_ARTIFACT_NODE_MODULES) {
    return process.env.CODEX_ARTIFACT_NODE_MODULES;
  }
  if (process.env.CODEX_RUNTIME_DEPENDENCIES) {
    return path.join(
...
function findArtifactToolPackage() {
  ...
  if (isArtifactToolPackage(localPackage)) {
    return validateArtifactToolPackage(localPackage, "the presentation QA workspace @oai/artifact-tool package");
  }
