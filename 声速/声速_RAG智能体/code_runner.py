...
# 两个专题智能体使用同一套安全策略；本文件保持本地副本，以支持各自独立打包。
import ast
import os
import re
import shutil
...
SENSITIVE_CODE_PATTERNS = (
    r"\bos\.environ\b", r"\bos\.getenv\s*\(", r"\bgetenv\s*\(", r"\benviron\.",
    r"\bsubprocess\.", r"\bprintenv\b", r"\bset\b", r"\bst\.secrets\b",
...
try:
    ...
    import os as _os
    if not any(_name.lower().endswith((".gif", ".mp4", ".webm")) for _name in _os.listdir(".")):
...
def _child_command(script_path: Path) -> list[str]:
    if getattr(sys, "frozen", False):
        return [sys.executable, "--python-snippet", str(script_path)]
...
def run_python_block(code: str, output_root: str | os.PathLike, timeout: int = 60) -> dict:
    ...
    if not is_safe:
        return {"returncode": None, "stdout": "", "stderr": "", "visuals": [], "run_dir": "",
    ...
    env = {
        "MPLBACKEND": "Agg", "PYTHONIOENCODING": "utf-8", "PATH": os.environ.get("PATH", ""),
        "SystemRoot": os.environ.get("SystemRoot", ""), "WINDIR": os.environ.get("WINDIR", ""),
        "TEMP": os.environ.get("TEMP", ""), "TMP": os.environ.get("TMP", ""),
    }
    mpl_config_dir = run_dir / "mplconfig"
    mpl_config_dir.mkdir(exist_ok=True)
    env["MPLCONFIGDIR"] = str(mpl_config_dir)
    proc = subprocess.run(_child_command(script_path), cwd=str(run_dir), env=env,
                          capture_output=True, text=True, timeout=timeout)
    visuals: list[Path] = []
    for pattern in ("*.gif", "*.png", "*.jpg", "*.jpeg", "*.mp4", "*.webm"):
        visuals.extend(path for path in run_dir.glob(pattern) if path.is_file())
    ...
    return {
        "returncode": proc.returncode, "stdout": stdout[-OUTPUT_PREVIEW_CHARS:],
        "stderr": stderr[-OUTPUT_PREVIEW_CHARS:], "stdout_truncated": len(stdout) > OUTPUT_PREVIEW_CHARS,
        "stderr_truncated": len(stderr) > OUTPUT_PREVIEW_CHARS,
        "stdout_path": str(stdout_path) if stdout else "", "stderr_path": str(stderr_path) if stderr else "",
        "visuals": [str(path) for path in sorted(visuals)], "run_dir": str(run_dir), "blocked": False,
    }
