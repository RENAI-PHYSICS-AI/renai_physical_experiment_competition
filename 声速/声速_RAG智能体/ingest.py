...
def infer_metadata(path: Path) -> dict:
    ...
    topic = "声速基础理论"
    lowered = stem.lower()
    if any(key in lowered for key in ("laplace", "kundt_regnault")):
        language = "fr"
    if any(
        key in lowered
        for key in ("echo", "time_of_flight", "time of flight", "回声", "时间差", "altitude")
    ):
        topic = "时间差与回声法"
    elif any(key in lowered for key in ("phase", "ultrasound", "超声", "相位", "水中声速")):
        topic = "相位与超声测量"
    elif any(
        key in lowered
        for key in ("kundt", "resonance", "standing", "驻波", "共振", "cardboard_tube")
    ):
        topic = "驻波与共振法"
    elif any(
        key in lowered
        for key in ("history", "colladon", "regnault", "马大猷", "中国声学", "museum")
    ):
        topic = "历史与经典实验"
    elif any(
        key in lowered
        for key in ("metamaterial", "sonic_material", "吸声", "time_reversal", "brillouin", "布里渊")
    ):
        topic = "现代声学拓展"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="构建声速测量文献 RAG 索引")
