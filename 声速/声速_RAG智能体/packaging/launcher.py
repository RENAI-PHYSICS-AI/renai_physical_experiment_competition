...
def log_dir() -> Path:
    directory = Path(os.getenv("LOCALAPPDATA", Path.home())) / "SoundSpeedExperimentTutor" / "logs"
...
def application_environment(
    ...
    environment.update(
        {
            "SOUND_SPEED_APP_DIR": str(root / "app"),
            "SOUND_SPEED_JULIA_EXE": str(
                root / "julia_app" / "bin" / "SoundSpeedWebRuntime.exe"
            ),
            "SOUND_SPEED_JULIA_PROJECT_DIR": str(root / "julia_app"),
            "SOUND_SPEED_WEB_HOST": "127.0.0.1",
            "SOUND_SPEED_WEB_BROWSER_HOST": "127.0.0.1",
            "SOUND_SPEED_WEB_PORT": str(julia_port),
    ...
    if heartbeat_url:
        environment["SOUND_SPEED_HEARTBEAT_URL"] = heartbeat_url
    environment.pop("SOUND_SPEED_JULIA_SCRIPT", None)
...
def run_streamlit_child() -> None:
    if not attach_current_process_to_named_job():
        raise SystemExit(3)
    root = bundle_root()
    streamlit_port = env_int("SOUND_SPEED_STREAMLIT_PORT", DEFAULT_STREAMLIT_PORT)
...
def main() -> int:
    ...
    process: subprocess.Popen | None = None
    browser_session: BrowserSession | None = None
    job_name = f"SoundSpeedTutorJob_{os.getpid()}_{uuid.uuid4().hex}"
    ...
    ack_path = (
        ...
        / f"sound_speed_job_ack_{os.getpid()}_{uuid.uuid4().hex}.txt"
