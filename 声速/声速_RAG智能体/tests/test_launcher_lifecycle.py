...
class LauncherLifecycleTests(unittest.TestCase):
    def test_port_pools_are_reserved_for_sound_speed(self) -> None:
        self.assertEqual(launcher.DEFAULT_STREAMLIT_PORT, 8502)
        self.assertEqual(launcher.DEFAULT_JULIA_PORT, 9385)
        self.assertEqual(launcher.STREAMLIT_FALLBACK_PORTS, range(28502, 28552))
        self.assertEqual(launcher.JULIA_FALLBACK_PORTS, range(29385, 29435))
        self.assertTrue(
        ...
        self.assertTrue(own_ports.isdisjoint(lissajous_ports))
    ...
    def test_export_directory_is_persistent_and_passed_to_child(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            configured = Path(temporary) / "实验导出" / "声速测量"
            with patch.dict(
                launcher.os.environ,
    ...
    def test_busy_preferred_ports_use_dedicated_fallback_pools(self) -> None:
        ...
        with (
            patch.dict(
                launcher.os.environ,
                {
                    "SOUND_SPEED_STREAMLIT_PORT": "42001",
                    "SOUND_SPEED_WEB_PORT": "42001",
                },
                clear=False,
            ),
            patch.object(
    ...
    def test_external_port_override_cannot_leave_assigned_pool(self) -> None:
        with (
            patch.dict(
                launcher.os.environ,
                {
                    "SOUND_SPEED_STREAMLIT_PORT": "18501",
                    "SOUND_SPEED_WEB_PORT": "19384",
                },
                clear=False,
            ),
