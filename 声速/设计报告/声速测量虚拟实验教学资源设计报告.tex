...
\end{equation}
若单次长度读数的绝对不确定度近似不变，跨越多个间隔会使 $D_q$ 增大，从而降低相对读数误差；这就是“逐差法/累积测量”的物理依据。Kundt 管和智能手机声管实验为波节与共振测量的课堂实现提供了参考\cite{kundt1868,hellesund2018,standing2024}。
...
输出与验证 & 显示错误与降级信息，组织问答和 Python 绘图产物 & 导出 CSV/PNG、自检四种反演链并提供确定性中间量\\
\bottomrule
\end{tabularx}
\end{table}

单文件启动器首先为 Streamlit 与 Julia 服务分别选择未占用的本机端口，将端口、资源目录和日志目录写入环境变量，再启动 Python 网页子进程。进入“演示实验”后，Python 按需拉起已封装的 Julia 网页运行时，并以 TCP 监听状态作为轻量就绪信号；确认端口建立后，页面中的内嵌框架才访问 Julia/WGLMakie 实验。此处使用本机回环通信，用户无需看到或手工输入第二个端口。实验内部的滑块、按钮、动画和曲线由浏览器与 Julia/Bonito 直接交换状态，不经 Python 逐帧中转，因此能够降低大数组复制和界面阻塞的风险。导出的 CSV 与 PNG 则构成语言无关的成果接口，可供 Python、表格软件或其他工具再次分析。
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.98\textwidth]{assets/sound_demo_interface.png}
  \caption{声速四种测量方法综合界面示例。图中为回声法：空间传播、脉冲信号和路程—时间关系同步显示。}
\end{figure}
...
\begin{sloppypar}
知识库由项目中的经典声学文献、现代测量资料和自研实验说明构建，涵盖 Laplace 声速修正和 Rayleigh 波动理论\cite{laplace1816,rayleigh1877}、回声与时间飞行\cite{berg2011,carvalho2007}、Kundt 管和超声相位/驻波\cite{kundt1868,water2020}，以及局域共振声学材料\cite{liu2000,masheng2016}。导入阶段对 PDF 逐页提取、清洗并重叠切块，保存文献名、页码、语言和“声速基础、时间差、相位、驻波、历史、现代拓展”等主题元数据。相比直接把整篇 PDF 交给模型，分页切块既能降低无关上下文，也使检索结果保留可回溯位置。
\end{sloppypar}
...
\node[box,left=of context] (tool) {四种方法确定性复算};
...
\end{tikzpicture}
\caption{声速专题问答的数据链路。虚线表示含明确测量参数的问题可触发确定性计算。}
\end{figure}
...
项目自带 8 道跨主题检索评估题，覆盖回声与时间差、相位与超声、驻波与共振、基础理论、历史实验、现代声学拓展及综合教学。2026 年 8 月 7 日在当前索引上运行评估脚本，8 道题的前 5 条结果均包含预设主题，主题命中率为 $8/8=100\%$。该指标只说明检索阶段找到相关主题，不能替代回答正确性和教学效果评价。提交前还需按表~\ref{tab:sound-rag-eval} 完成回答级、图像级和教学级测试。
...
依据一致性 & 检查回答是否超出检索材料或混淆理论值与测量值 & \todoitem{补充人工核查结果}\\
图像分析可靠性 & 用清晰、模糊、缺刻度三组波形/装置图片测试 & \todoitem{补充识别与拒答案例}\\
安全拦截 & 测试允许绘图与文件、网络、密钥、进程等危险代码 & 已有自动化单元测试；\todoitem{补充最终测试记录}\\
教学效果 & 比较有/无助教条件下方法选择、误差解释和任务时间 & \todoitem{补充真实教学数据}\\
\bottomrule
\end{tabularx}
\end{table}
...
回答级评价拟采用“物理正确性、适用条件、单位与量纲、依据一致性、拒答恰当性、教学可操作性”六个维度。对于计算题，应将解析结果和工具结果分别保存；对于方法比较题，应检查是否说明比较条件；对于图像题，应记录系统是否在缺少刻度时拒绝给出虚假精确值。通过建立错误类型标签，可以把“回答不理想”进一步分解为检索遗漏、条件缺失、单位错误、整周判断错误或表达不清，为后续改进提供明确依据。
...
程序包含四种模型的自检，并保存各模式静态渲染截图。提交前应在最终单文件版本上重新记录测试环境和结果：\todoitem{填写操作系统、浏览器、启动时间、各模式通过情况、导出文件复算结果与异常处理记录}。
...
\end{enumerate}
\todoitem{填写样本量、分组、评价工具、统计方法和真实数据；不得以仿真准确度代替教学效果。}
...
\begin{enumerate}
  ...
  \item WGLMakie 首次解包和初始化可能较慢，且浏览器需支持WebGL；后续可增加启动缓存和轻量后备显示。
...
\begin{itemize}
  ...
  \item 送审版文件大小、SHA-256、冷启动/热启动时间和最低硬件：\todoitem{实测后填写}。
\end{itemize}

单文件首次启动需解压资源、建立运行目录并初始化 Julia/WGLMakie，耗时会高于后续启动。启动器应持续显示当前阶段、网页地址和日志位置，使用户能够判断程序正在工作而不是重复启动。缓存、知识库、用户导出数据和日志采用不同目录：缓存可安全重建，知识库随版本发布，导出数据由用户保留，日志用于排障。这样的目录划分既支持清理，也避免升级或重新封装时误删实验成果。
...
匿名编号 & 主要贡献 & 可核验证据\\
\midrule
成员A & \todoitem{声学模型、文献调研或实验设计} & \todoitem{提交记录/实验记录}\\
成员B & \todoitem{Julia交互可视化与算法开发} & \todoitem{提交记录/测试记录}\\
成员C & \todoitem{知识库、智能问答或前端开发} & \todoitem{提交记录/测试记录}\\
成员D & \todoitem{验证、视频、报告与教学评价} & \todoitem{实验记录/素材清单}\\
成员E & \todoitem{如无第五名则删除本行} & \todoitem{证据}\\
\bottomrule
\end{tabularx}
\caption{贡献与历程必须据实填写，并与代码提交、实验记录和素材版本对应。}
\end{table}
...
\bibitem{laplace1816} P.-S. Laplace. Sur la vitesse du son dans l'air et dans l'eau. \emph{Annales de Chimie et de Physique}, 1816.
\bibitem{rayleigh1877} J. W. Strutt (Lord Rayleigh). \emph{The Theory of Sound}. 1877.
\bibitem{morse1948} P. M. Morse. \emph{Vibration and Sound}. McGraw-Hill, 1948.
\bibitem{crawford1968} F. S. Crawford. \emph{Waves: Berkeley Physics Course, Vol. 3}. McGraw-Hill, 1968.
\bibitem{carvalho2007} C. C. Carvalho, J. M. B. Lopes dos Santos, and M. B. Marques. A time-of-flight method to measure the speed of sound using a stereo sound card. 2007.
\bibitem{staacks2018} S. Staacks et al. Simple time-of-flight measurement of the speed of sound using smartphones. 2018.
\bibitem{berg2011} A. Berg and M. Courtney. Echo-based measurement of the speed of sound. 2011.
\bibitem{hellesund2018} S. Hellesund. Measuring the speed of sound in air using a smartphone and a cardboard tube. 2018.
\bibitem{kundt1868} A. Kundt and H. V. Regnault. Historical studies on sound velocity and Kundt-tube standing waves. \emph{Archives des sciences physiques et naturelles}, 1868.
\bibitem{water2020} 水中声速测量实验的模型修正. \emph{物理实验}, 40(8):40--45, 2020.
\bibitem{sjtuultra} 赵西梅、李向亭、周红、沈学浩、杨卫群. 超声波声速测量的拓展实验. 上海交通大学物理实验教学中心.
\bibitem{liu2000} Z. Liu et al. Locally resonant sonic materials. \emph{Science}, 289:1734--1736, 2000.
\bibitem{masheng2016} G. Ma and P. Sheng. Acoustic metamaterials: From local resonances to broad horizons. \emph{Science Advances}, 2:e1501595, 2016.
\bibitem{tjnu} 天津师范大学. 大学物理实验：声速的测量与测定.
\bibitem{standing2024} 金靓婕等. 驻波法测声速原理教学的讨论. 2024.
\bibitem{course2023} 教育部高等学校大学物理课程教学指导委员会. \emph{理工科类大学物理实验课程教学基本要求（2023）}.
\bibitem{software} JuliaLang contributors. Julia 1.10 documentation; Makie, WGLMakie, Bonito, PackageCompiler, Streamlit and PyInstaller project documentation. 访问日期：\todoitem{填写}.
\end{thebibliography}
