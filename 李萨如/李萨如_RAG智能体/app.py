...
QUICK_QUESTION_POOL = (
    ...
    "为什么实验中的李萨如图形会缓慢旋转？",
    "如何用示波器和李萨如图形测量未知频率？",
    "频率比为 1:1 时，什么条件下轨迹是圆？",
    "李萨如图形测相位差时有哪些主要误差来源？",
    "怎样设计一个可复现的李萨如图形实验？",
    "频率比不是有理数时，轨迹为什么不能闭合？",
    "Melde 的振动实验与李萨如图形有什么联系？",
    "如何利用演示实验观察相位差对轨迹方向的影响？",
)
...
def install_launcher_heartbeat() -> None:
    ...
    components.html(
        ...
        (() => {{
            ...
            if (host.__lissajousShowHandler) {{
                host.removeEventListener("pageshow", host.__lissajousShowHandler);
            }}
            const lifecycle = host.__lissajousLifecycle || {{
                client: host.crypto && host.crypto.randomUUID
                    ? host.crypto.randomUUID()
                    : `${{Date.now()}}-${{Math.random().toString(16).slice(2)}}`,
                seq: 0,
            }};
            host.__lissajousLifecycle = lifecycle;
            const eventUrl = (base) => {{
                lifecycle.seq += 1;
            ...
            const close = (event) => {{
                ...
                if (host.__lissajousHeartbeatTimer) {{
                    host.clearInterval(host.__lissajousHeartbeatTimer);
                    host.__lissajousHeartbeatTimer = null;
                }}
                let queued = false;
            ...
            host.__lissajousShowHandler = resume;
...
def set_stream_autoscroll(enabled: bool) -> None:
    if enabled:
        ...
        (() => {
            ...
            const previous = host.__lissajousStreamScroll;
            if (previous) {
            ...
            host.__lissajousStreamScroll = state;
            scheduleScroll();
        })();
        </script>
        """
        ...
        (() => {
            const host = window.parent;
            const state = host.__lissajousStreamScroll;
            if (!state) return;
...
def enable_clipboard_image_paste() -> None:
    ...
    (() => {
        const host = window.parent;
        const doc = host.document;
        if (host.__lissajousPasteHandler) {
            doc.removeEventListener("paste", host.__lissajousPasteHandler, true);
        }
        ...
        host.__lissajousPasteHandler = handler;
...
header_image = Path(__file__).resolve().parent / "assets" / "lissajous_header.png"
header_image_data = base64.b64encode(header_image.read_bytes()).decode("ascii")
st.markdown(
    f"""
    <section class="app-hero">
        <div>
            <p class="app-hero__kicker">LISSAJOUS FIGURES · VIBRATION LAB</p>
            <h1>李萨如图形实验智能助教</h1>
            <p class="app-hero__subtitle">正交振动合成 · 相位与频率 · 示波器测量 · 可视化实验</p>
        </div>
        <div class="app-hero__visual">
            <img src="data:image/png;base64,{header_image_data}" alt="3:2 频率比的李萨如轨迹">
        </div>
    </section>
    <p class="app-lede">从正交简谐振动出发，理解相位差与频率比如何塑造轨迹，并把图形带回真实实验观测。</p>
    """,
    unsafe_allow_html=True,
)
chat_tab, demo_tab = st.tabs(["智能问答", "演示实验"])
...
with chat_tab:
    ...
    if not st.session_state.messages:
        st.markdown(
            ...
            <section class="chat-guide">
                <p class="chat-guide__title">围绕图形、测量与实验展开</p>
                <p class="chat-guide__summary">从振动方程到示波器轨迹，逐步连接物理原理、实验现象与数据分析。</p>
                <ul>
                    <li>频率比、相位差与轨迹形状</li>
                    <li>示波器测频、测相与图形判读</li>
                    <li>实验设计、误差来源与改进方法</li>
                </ul>
            </section>
            """,
            unsafe_allow_html=True,
    ...
    if images and not question:
        question = "请分析这些图片，并结合李萨如图形实验说明其中的物理现象。"
    if question:
        user_message = {
            "role": "user",
            "content": question,
...
with demo_tab:
    ...
    selected_experiment = st.segmented_control(
        "实验",
        list(experiment_routes),
        default="相位差",
