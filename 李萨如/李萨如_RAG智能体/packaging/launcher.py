...
# Dedicated fallback pools keep this application disjoint from the sound-speed
# tutor even when both default port pairs are already occupied.
STREAMLIT_FALLBACK_PORTS = range(18501, 18551)
JULIA_FALLBACK_PORTS = range(19384, 19434)
HEARTBEAT_PORTS = range(19850, 19900)
...
EXPORT_DIR_ENV = "LISSAJOUS_EXPORT_DIR"
EXPORT_SUBDIR = "李萨如图形"
...
def export_dir() -> Path:
    ...
    for candidate in candidates:
        ...
        if prepared is not None:
            return prepared
    raise RuntimeError("无法创建李萨如实验导出目录。")
...
def select_service_ports() -> tuple[int, int]:
    ...
    julia_preferred = configured_port(
        "LISSAJOUS_WEB_PORT", DEFAULT_JULIA_PORT, JULIA_FALLBACK_PORTS
    )
