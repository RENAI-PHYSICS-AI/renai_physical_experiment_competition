...
\end{equation}
当 $|\Delta f|\ll f$ 时，一个载波周期内相对相位仅改变约 $2\pi\Delta f/f$，轨迹可近似看作由式\eqref{eq:liss-conic}描述的“瞬时椭圆”；经过较长时间，其取向和扁率又随式\eqref{eq:liss-detune-phase}缓慢变化。程序用进度参数定位 $0$ 至 $T_{\mathrm{shape}}$ 的观察时刻，仅绘制该时刻附近一个载波周期的局部轨迹，同时给出未折返的累计相位和映射到 $0^\circ$--$360^\circ$ 的等效相位。$\Delta f$ 的符号控制演化方向，绝对值控制快慢；当 $\Delta f=0$ 时不存在有限形态周期，程序改用两个载波周期作为观察范围。机械振动和谐振扫描研究也表明，微小频漂和相位延迟会造成轨迹的慢旋转或成像失配\cite{denhartog1956,birla2020,janin2022}。
...
知识库由项目中的经典文献、现代研究资料和自研实验说明构建，内容从十九世纪振动曲线与驻波研究延伸到示波器教学、二维谐振子和 MEMS 扫描\cite{terquem1878,melde1860,cena2014,hogan2019,wang2020,birla2020,huang2025}。导入阶段执行 PDF 分页提取、文本清洗、重叠切块，并保存文献名、页码、年份、语言和主题等元数据。对于 Melde 1860 年扫描专篇，项目另行建立逐页德文 OCR 文字层；扫描质量不足的页面记录在提取报告中，不用猜测文字填补空缺。
...
\bibitem{terquem1878} A. Terquem. Sur les courbes dues à la coexistence de deux mouvements vibratoires perpendiculaires. 1878.
\bibitem{melde1860} F. Melde. Über die Erregung stehender Wellen eines fadenförmigen Körpers. \emph{Annalen der Physik}, 1860.
\bibitem{doll2007} R. Doll and G.-L. Ingold. Lissajous curves and semiclassical theory: The two-dimensional harmonic oscillator. \emph{American Journal of Physics}, 75:208--216, 2007.
\bibitem{cena2014} C. R. Cena et al. Theoretical and experimental approach to the harmonic oscillator using the Lissajous curves. \emph{Revista Brasileira de Ensino de Física}, 36(2):2302, 2014.
\bibitem{denhartog1956} J. P. Den Hartog. \emph{Mechanical Vibrations}. McGraw-Hill, 1956.
\bibitem{hogan2019} T. Hogan. Oscilloscope measurements using Lissajous figures. 2019.
\bibitem{wang2020} J. Wang, G. Zhang, and Z. You. Design rules for dense and rapid Lissajous scanning. \emph{Microsystems \& Nanoengineering}, 2020.
\bibitem{barna2020} I. F. Barna and L. Mátyás. Lissajous curves with a finite sum of prime number frequencies. arXiv:2009.01687, 2020.
\bibitem{birla2020} M. Birla et al. Image processing metrics for phase identification of a multi-axis MEMS scanner used in single pixel imaging. \emph{IEEE/ASME Transactions on Mechatronics}, DOI:10.1109/TMECH.2020.3020923, 2020.
\bibitem{janin2022} P. Janin, D. Uttamchandani, and R. Bauer. On-chip frequency tuning of fast resonant MEMS scanner. 2022.
\bibitem{huang2025} 黄张弛等. 基于数字示波器进行图像重建的原理和方法. \emph{物理与工程}, 35(3), 2025.
\bibitem{uestc} 电子科技大学. 大学物理实验：示波器李萨如图形测频率与相位.
\bibitem{tongji} 同济大学. 物理实验手册：示波器与李萨如图形.
